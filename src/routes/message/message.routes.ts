...
router.post(
  '/',
  invalidateCacheByNamespace(['messages', 'notifications', 'dashboard', 'analytics']),
...
router.get(
  ...
  cacheGet({ namespace: 'messages', ttlSeconds: 90 }),
...
router.patch(
  ...
  jwtVerify,
  roleCheck(['admin', 'manager']),
  invalidateCacheByNamespace(['messages', 'dashboard', 'analytics']),
