...
export class AuthController {
  ...
  static async signup(req: Request, res: Response) {
    try {
      ...
      if (
        e.message === Message.REGISTRATION_LIMIT_EXCEEDED ||
        e.message === Message.REGISTRATION_DEVICE_LIMIT_EXCEEDED
      ) {
      ...
      if (e.message === 'Database not initialized') {
        return res
          ...
          .json({ message: 'Server not ready. Please try again shortly.' });
      }
      ...
      return res
        .status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
        .json({ message: e.message || Message.INTERNAL_SERVER_ERROR });
    }
  }
  ...
  static async signin(req: Request, res: Response) {
    try {
      ...
      const { tokens, user, revokedSessionId } = await authService.signin(
        dto,
      ...
      return res
        ...
        .json({ message: message || Message.INVALID_EMAIL_OR_PASSWORD });
    }
  }
  ...
  static async forgotPassword(req: Request, res: Response) {
    try {
      ...
      const authService = AuthController.createAuthService();
      const result: ServiceResult<{ resetUrl?: string } | null> = await authService.forgotPassword(
        dto.email,
      );
      ...
      const responseBody: { message: string; debug?: { resetUrl?: string } } = {
  ...
  static async resetPassword(req: Request, res: Response) {
    try {
      ...
      const authService = AuthController.createAuthService();
      const result: ServiceResult<null> = await authService.resetPassword(
        dto.email,
        dto.token,
        dto.password,
      );
