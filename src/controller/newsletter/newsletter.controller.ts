    ...
    try {
      ...
      if (!subject || !headline) {
        return res
          ...
          .json({ message: 'subject and headline are required.' });
      }
      ...
      if (eventsInput !== undefined && !Array.isArray(eventsInput)) {
        return res
          .status(HTTP_STATUS.BAD_REQUEST)
          .json({ message: 'events must be an array of strings.' });
      }
      ...
      const result = await newsletterService.sendCampaign({
        ...
        offerTitle:
          typeof req.body?.offerTitle === 'string' ? req.body.offerTitle.trim() : undefined,
        offerDescription:
          typeof req.body?.offerDescription === 'string'
            ? req.body.offerDescription.trim()
            : undefined,
        events,
        couponCode:
          typeof req.body?.couponCode === 'string' ? req.body.couponCode.trim() : undefined,
        validUntil:
          typeof req.body?.validUntil === 'string' ? req.body.validUntil.trim() : undefined,
