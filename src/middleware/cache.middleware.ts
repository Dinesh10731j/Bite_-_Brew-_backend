...
export const cacheGet =
  (options: CacheOptions) => async (req: Request, res: Response, next: NextFunction) => {
...
export const invalidateCacheByNamespace =
  (namespaces: string[]) => async (_req: Request, _res: Response, next: NextFunction) => {
    if (!CACHE_ENABLED) {
      return next();
    }
