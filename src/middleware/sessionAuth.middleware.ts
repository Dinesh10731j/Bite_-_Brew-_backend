...
// Device fingerprint check (risk signal, not sole auth factor).
    if (envConfig.DEVICE_FINGERPRINT_ENFORCED && validation.session?.deviceHash) {
      const reqDeviceHash = deviceService.buildDevice({
        visitorId: typeof req.headers["x-device-id"] === "string" ? req.headers["x-device-id"] : undefined,
      ...
      if (reqDeviceHash !== validation.session.deviceHash) {
        res.status(HTTP_STATUS.UNAUTHORIZED).json({ message: "Device mismatch. Please sign in again." });
        return;
      }
    }
    ...
    req.session = { sessionId, userId: user.id, ipAddress: resolveClientIp(req), deviceHash: req.deviceHash };
