...
export class RefreshTokenService {
  ...
  async createRefreshToken(userId: string, sessionId: string): Promise<CreatedRefreshToken> {
    ...
    const token = jwt.sign({ userId, sessionId, tokenId }, REFRESH_SECRET as jwt.Secret, {
    ...
    await securityRedis.set(
      'refresh',
      tokenId,
      JSON.stringify({ userId, sessionId, tokenHash }),
      this.ttlSeconds(expiresIn),
    );
  ...
  async verifyAndRotate(
    refreshToken: string,
  ): Promise<{ userId: string; sessionId: string; newToken: CreatedRefreshToken }> {
